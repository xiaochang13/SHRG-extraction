...
def depthFirstPi(amr, tok_seq, all_alignments, sorted_indexes, print_info=False):
    ...
    while stack:
        ...
        for edge_index in reversed(curr_node.v_edges):
            ...
            if curr_edge.label in exclude_rels:
                ...
                if curr_edge.label != 'name':
                    ...
                    for next_edge_index in reversed(tail_node.v_edges):
                        ...
                        edge_labels[(curr_node_index, next_child_index)] = next_edge.label
            ...
            edge_labels[(curr_node_index, child_index)] = curr_edge.label
            edge_list.append((curr_node_index, child_index))
    ...
    return topo_list, edge_map, edge_list, edge_labels


# Traverse AMR from top down, also categorize the sequence in case of alignment existed
def buildPiSeq(amr, sorted_indexes):
    ...
    while stack:
        ...
        for edge_index in reversed(curr_node.v_edges):
            ...
            old_edges[(curr_node_index, child_index)] = curr_edge.label
    ...
    # Map original vertices to a vertex order
    pi_map = {}
    for (i, v) in enumerate(topo_list):
        pi_map[v] = i

    # Re-organize edges according to vertex order.
    edge_label_map = {}
    for (node_idx, tail_idx) in old_edges:
        start_id = pi_map[node_idx]
        end_id = pi_map[tail_idx]

        # Save the re-organized edge maps.
        edge_map[start_id].add(end_id)
        edge_map[end_id].add(start_id)

        curr_lab = old_edges[(node_idx, tail_idx)]
        if start_id < end_id:
            arc_lab = "R-" + curr_lab
            edge_label_map[((start_id, end_id))] = arc_lab
        else:
            arc_lab = "L-" + curr_lab
            edge_label_map[(end_id, start_id)] = arc_lab

    if len(vertex_set) != len(topo_list):
        uncovered = set(topo_list) - vertex_set
        for index in uncovered:
            print amr.nodes[index].node_str()
    return topo_list, edge_map, edge_label_map

def buildRightNeighbors(n, edge_map):
        ...
        for j in edge_map[i]:
            if j > i:
                ...
                if j > right_most:
                    right_most = j
        if right_most != -1:
            right_most_map[i] = right_most
    return right_map, right_most_map

def ordered_edges(pi_seq, pi_map, edge_map):
    all_edges = []
    for (i, curr) in enumerate(pi_seq):
        assert pi_map[curr] == i
        for next in edge_map[curr]:
            j = pi_map[next]
            if j > i:
                all_edges.append("%d-%d" % (i, j))
    return all_edges
