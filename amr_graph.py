...
class AMREdge(object):
    ...
    def set_coref(self, val):
        self.is_coref = val
        if val and self.graph:
            self.graph.has_reent = True
...
class AMRGraph(object):
    ...
    def build_edge_maps(self):
        ...
        while stack:
            curr_node_idx, par_rel, depth, is_coref = stack.pop()
            sequence.append((curr_node_idx, is_coref, par_rel, depth)) #push a tuple recording the depth search info
            if is_coref or curr_node_idx in visited_nodes:
                continue
            ...
            if len(curr_node.v_edges) > 0:
                for edge_index in reversed(curr_node.v_edges):  #depth first search
                    ...
                    self.edge_map[curr_node_idx].add(child_index)
                    self.edge_map[child_index].add(curr_node_idx)
                    self.edge_labels[(curr_node_idx, child_index)] = curr_rel
                    self.edge_list.append((curr_node_idx, child_index))
    ...
    def collapsedAMR(cls, amr, all_alignments):
        ...
        def category_info(node_idx, category, subgraph=None):
            ...
            if category == "NER":
                ...
                exclude_rels = [s.split(":")[0] for s in all_rels]
                category_name = "MULT"
    ...
    def extract_entities(self):
        ...
        while stack:
            ...
            if curr_node.is_named_entity():
                frag = self.build_entity_fragment(curr_node, "NER")
    ...
    def build_entity_fragment(self, node, category="NONE"):
